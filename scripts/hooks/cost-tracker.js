...
const { ensureDir, appendFile, getClaudeDir, log } = require('../lib/utils');
...
function run(raw) {
  try {
    ...
    const metricsDir = `${getClaudeDir()}/metrics`;
    ...
    appendFile(`${metricsDir}/costs.jsonl`, `${JSON.stringify(row)}\n`);

    if (row.estimated_cost_usd > 1.0) {
      log('CostTracker', `Session cost: $${row.estimated_cost_usd.toFixed(4)} (${model})`);
    }
  } catch {
