#!/usr/bin/env node
/**
 * Central Hook Runner with Profile Gating
 *
 * All everypaper hooks route through this runner. It:
 * 1. Reads raw JSON from stdin (passed by Claude Code)
 * 2. Checks if hook is enabled via profile flags (minimal/standard/strict)
 * 3. Either require()s the hook module directly (if it exports run()),
 *    or spawns a child process for legacy hooks
 * 4. Passes stdin through on success, never blocks tool execution
 *
 * Usage:
...
async function main() {
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...process.env,
      EVERYPAPER_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      EVERYPAPER_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
