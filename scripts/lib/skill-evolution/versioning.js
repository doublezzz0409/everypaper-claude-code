...
function ensureSkillVersioning(skillPath) {
  ...
  return { versionsDir, evolutionDir };
}
...
function listVersions(skillPath) {
  ...
  return fs.readdirSync(versionsDir)
    .map(fileName => {
      ...
      return { version, path: filePath, created_at: stats.mtime.toISOString() };
    })
    .filter(Boolean)
...
function getCurrentVersion(skillPath) {
  const skillFilePath = getSkillFilePath(skillPath);
  if (!fs.existsSync(skillFilePath)) return 0;
  const versions = listVersions(skillPath);
  return versions.length === 0 ? 1 : versions[versions.length - 1].version;
}
...
function readJsonl(filePath) {
  ...
  return fs.readFileSync(filePath, 'utf8')
    ...
    .filter(Boolean)
    .reduce((rows, line) => {
      try { rows.push(JSON.parse(line)); } catch { /* ignore malformed */ }
      return rows;
    }, []);
}
...
function createVersion(skillPath, options = {}) {
  ...
  return { version: nextVersion, path: snapshotPath, created_at: createdAt };
}
