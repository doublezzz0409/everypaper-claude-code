...
function classifySkillPath(skillPath, options = {}) {
  ...
  if (isWithinRoot(skillDir, roots.curated)) return SKILL_TYPES.CURATED;
  if (isWithinRoot(skillDir, roots.learned)) return SKILL_TYPES.LEARNED;
  if (isWithinRoot(skillDir, roots.imported)) return SKILL_TYPES.IMPORTED;
...
function writeProvenance(skillPath, record, options = {}) {
  ...
  return { path: provenancePath, record: { ...record } };
}
